    ...
    if menu_category.isdigit():
        ...
        if int(menu_category) in menu_items.keys():
            ...
            if menu_selection.isdigit():
                ...
                quantity = input(
                    f"How many of the {item_name} would you like to order?"
                )
...
# print(order)
# print a formatted list of the customer's order
...
for item in order_list:
    ...
    # 8 Calculate the number of spaces needed for formatting
    num_item_spaces = 25 - len(item_name)
    # 9 Create space strings
    item_spaces = " " * num_item_spaces
    ##10 Print the formatted order item
    print(f"{item_name}{item_spaces} | ${price:5.2f} | {quantity:8}")
    # 11. Calculate the cost of the order using list comprehension
    prices = [item["Price"] * item["Quantity"] for item in order_list]
# Multiply the price by quantity for each item in the order list, then sum()
# and print the prices.
print(f"The total cost of your order is: ${sum(prices):.2f}, have a good day!")
